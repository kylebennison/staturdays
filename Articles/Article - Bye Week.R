...
# Will use 14 days, to only get normal regular season skip a saturday bye weeks
bye_week_schedule <- schedule_combined %>% 
  filter(week < 16) %>% 
  mutate(bye_week = if_else(game_date - lag(game_date, n = 1L, order_by = game_date) == 14, 1, 0))

pval <- round(as.numeric(prop.test(x = c(14723, 1226), n = c(29309, 2514))[3]), 2)

# Get avg result grouped by bye week
bye_vs_reg_plt <- bye_week_schedule %>% 
  group_by(bye_week) %>% 
  summarise(win_rate = mean(game_outcome), count_wins = sum(game_outcome), count = n()) %>% 
  filter(is.na(bye_week) == F) %>% 
  ggplot(aes(x = as.factor(bye_week), y = win_rate, fill = as.factor(bye_week))) +
  geom_col(width = .5, alpha = 0.7) +
  labs(title = "Average Win Rate after Bye Week",
       subtitle = paste0("P-value: ", pval, " - No significant difference"),
       caption = "@staturdays | @kylebeni012 - Data: @cfb_data",
       x = "Week Type",
       y = "Win %") +
  staturdays_theme +
  scale_x_discrete(labels = c("Regular", "Coming Off Bye")) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  scale_fill_manual(values = c("#de703b", "#041e42")) +
  theme(legend.position = "none") +
  annotation_custom(logo, xmin = 2.5, xmax = 3, ymin = -.15, ymax = .15) +
  coord_cartesian(clip = "off") +
  theme(plot.margin = unit(c(1,10,1,1), "lines"))

ggsave(filename = paste0("bye_vs_reg_plt_", str_replace_all(now(), ":", "."), ".png"), 
       plot = bye_vs_reg_plt,
       path = "C:/Users/Kyle/Documents/Kyle/Staturdays/R Plots",
       dpi = 300, width = 400, height = 200, units = "mm")

# Control for opponent - still no significance
bye_week_opp_elo_tbl <- bye_week_schedule %>% 
  mutate(elo_bucket = round(elo_diff, -2)) %>% 
  group_by(bye_week, elo_bucket) %>% 
  summarise(win_rate = mean(game_outcome), count_wins = sum(game_outcome), count = n()) %>% 
  pivot_wider(names_from = bye_week, values_from = c(win_rate, count_wins, count), names_prefix = c("regular_", "bye_")) %>% 
  select(!(dplyr::contains("NA"))) %>% 
  filter(!is.na(count_bye_1)) %>% 
  filter(count_bye_1 > 30) %>% 
  group_by(elo_bucket) %>% 
  mutate(signif = as.numeric(prop.test(n = c(count_regular_0, count_bye_1), x = c(count_wins_regular_0, count_wins_bye_1))[3]))

# Control for opponent plot
bye_week_opp_elo_plt <- bye_week_opp_elo_tbl %>% 
  pivot_longer(cols = c(2,3), names_to = "bye_or_reg", values_to = "win_rate") %>% 
  ggplot(aes(x = elo_bucket, y = win_rate)) +
  geom_smooth(aes(color = bye_or_reg)) +
  labs(title = "Bye-week effects",
       subtitle = "2000-2019, controlled for opponent, minimum 30 bye weeks",
       caption = "@staturdays | @kylebeni012 - Data: @cfb_data",
       x = "Opponent Elo Differential",
       y = "Win %") +
  staturdays_theme +
  scale_x_continuous() +
  scale_y_continuous(labels = percent) +
  scale_color_manual(values = c("#de703b", "#041e42"), 
                     labels = c("After Bye Week", "Regular Week"),
                     name = "Week Type") +
  annotation_custom(logo, xmin = 600, xmax = 1000, ymin = -.2, ymax = .1) +
  coord_cartesian(clip = "off")

ggsave(filename = paste0("bye_week_opp_elo_plt_", str_replace_all(now(), ":", "."), ".png"), 
       plot = bye_week_opp_elo_plt,
       path = "C:/Users/Kyle/Documents/Kyle/Staturdays/R Plots",
       dpi = 300, width = 400, height = 200, units = "mm")

# Group by team
# get conf
conferences <- elo_conf %>% 
  filter(season == 2019, week == 0) %>% 
  select(team, conference) %>% unique()

bye_week_team <- bye_week_schedule %>% 
  mutate(elo_bucket = round(elo_diff, -2)) %>% 
  group_by(bye_week, team) %>% 
  summarise(win_rate = mean(game_outcome), count_wins = sum(game_outcome), count = n()) %>% 
  pivot_wider(names_from = bye_week, values_from = c(win_rate, count_wins, count), names_prefix = c("regular_", "bye_")) %>% 
  select(!(dplyr::contains("NA"))) %>% 
  filter(!is.na(count_bye_1)) %>% 
  filter(count_bye_1 > 14) %>% 
  group_by(team) %>% 
  mutate(signif = as.numeric(prop.test(n = c(count_regular_0, count_bye_1), x = c(count_wins_regular_0, count_wins_bye_1))[3]))

bye_min <- bye_week_team %>% pull(win_rate_bye_1) %>% min()
bye_max <- bye_week_team %>% pull(win_rate_bye_1) %>% max()
reg_min <- bye_week_team %>% pull(win_rate_regular_0) %>% min()
reg_max <- bye_week_team %>% pull(win_rate_regular_0) %>% max()

# Plot team logos, x = regular win rate, y = bye win rate, see which teams are good off a bye
bye_week_team_plot <- bye_week_team %>% 
  left_join(conferences) %>% 
  filter(conference %in% power_5) %>% 
  left_join(team_colors, by = c("team" = "school")) %>% 
  ggplot(aes(x = win_rate_regular_0, y = win_rate_bye_1)) +
  geom_image(aes(image = light), asp = 2, alpha = 0.7) +
  geom_abline(linetype = "dashed", color = staturdays_colors("orange")) +
  theme(aspect.ratio = 1/2) +
  annotate(geom = "label", x = reg_max*.8, y = bye_max*.2, label = "Hurt by Bye Week",
           fill = staturdays_colors("orange"), color = "white", alpha = 0.8) +
  annotate(geom = "label", x = reg_max*.4, y = bye_max*.8, label = "Helped by Bye Week",
           fill = staturdays_colors("orange"), color = "white", alpha = 0.8) +
  labs(title = "Bye-week effects",
       subtitle = "2000-2019, minimum 14 bye weeks",
       caption = "@staturdays | @kylebeni012 - Data: @cfb_data",
  ...
  scale_y_continuous(labels = percent) +
  annotation_custom(logo, xmin = .82, xmax = 1.02, ymin = -.01, ymax = .39) +
  coord_cartesian(clip = "off") +
  theme(plot.margin = unit(c(1,3,1,1), "lines"))

ggsave(filename = paste0("bye_week_teams_", str_replace_all(now(), ":", "."), ".png"), 
       plot = bye_week_team_plot,
       path = "C:/Users/Kyle/Documents/Kyle/Staturdays/R Plots",
       dpi = 300, width = 400, height = 200, units = "mm")
