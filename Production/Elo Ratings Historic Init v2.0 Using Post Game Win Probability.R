...
cfb_games <- cfb_games %>% mutate(game_outcome_home = 
                                  ...
                                  home_post_win_prob = as.numeric(ifelse(is.na(home_post_win_prob), game_outcome_home,home_post_win_prob))
)
...
for(yr in c(2000:2021)){
  ...
  for(wk in c(1:max(cfb_games[which(cfb_games$season == yr),]$week))){
    ...
    if(nrow(current_week) != 0) {
      ...
      current_week <- current_week %>% mutate(new_home_rating = calc_new_elo_rating(home_rating, home_post_win_prob, calc_expected_score((home_rating+if_else(neutral_site == F, home_field_advantage, neutral_adjust)), away_rating),k),
                                              new_away_rating = calc_new_elo_rating(away_rating, 1-home_post_win_prob, calc_expected_score(away_rating, (home_rating+if_else(neutral_site == F, home_field_advantage, neutral_adjust))),k))
