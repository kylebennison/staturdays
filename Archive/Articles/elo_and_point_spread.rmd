...
```{r warning=FALSE}
ge_predictions %>% 
  mutate(error = abs(final_home_spread - pred_home_spread)) %>% 
  ggplot(aes(x = final_home_spread, y = pred_home_spread)) +
  geom_point(alpha = .1, aes(size = error),
             color = staturdays_colors("light_blue")) +
  geom_abline(color = staturdays_colors("orange"), linetype = 2) +
