...
def calc_rolling_results(df: pd.DataFrame, windows: Union[int, list]) -> pd.DataFrame:
    ...
    for window in windows:
        ...
        roll_cols = [
            x for x in roll.columns if "_sum" in x or "_mean" in x or "_std" in x
        ]

        new_names = [
            x + f"_{window}"
            for x in roll.columns
            if "_sum" in x or "_mean" in x or "_std" in x
        ]
    ...
    final_names = [
        x for x in final.columns if "_sum" in x or "_mean" in x or "_std" in x
    ]
...
def main(
    ...
    schedule = calc_rolling_results(df, [2, 4, 6, 12])
